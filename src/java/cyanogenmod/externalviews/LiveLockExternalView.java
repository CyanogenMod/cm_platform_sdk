        ...
        try {
            ...
            for (int indx = 0 ; indx < requestedPermissions.length ; indx++) {
                if ((requestedPermissionsFlags[indx] & PackageInfo.REQUESTED_PERMISSION_GRANTED) == 0) {
                    ...
                    try {
                        ...
                        Log.e(TAG,"PackageManager can't find a permission with name " + requestedPermissions[indx] + " and has not been granted!");
                    }
                } else {
            ...
            if (permissionsNotGranted.size() == 0) {
                ...
                if (resolveInfo.size() >= 1) {
                    ...
                    //No resolvers == no way to let the user know this
                    //package needs permissions in order to run properly
                    //assume no permissions required and hope for the best?
                }               
            }            
        } catch (NameNotFoundException e) {
