...
public class TorchManager {
    ...
    private static ITorchService sService;
    ...
    public void addListener(TorchCallback callback) {
        if (sService == null) {
            Log.w(TAG, "not connected to TorchService");
            return;
        }
        synchronized (mCallbacks) {
            ...
            if (mCallbacks.size() == 0) {
                try {
                    getService().addListener(mTorchChangeListener);
    ...
    public void removeListener(TorchCallback callback) {
        ...
        synchronized (mCallbacks) {
            ...
            if (mCallbacks.size() == 0) {
                try {
                    sService.removeListener(mTorchChangeListener);
                } catch (RemoteException e) {
    ...
    public void setTorchEnabled(final boolean newState) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    sService.setTorchEnabled(newState);
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
            }
    ...
    /** @hide */
    public ITorchService getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(CMContextConstants.CM_TORCH_SERVICE);
        sService = ITorchService.Stub.asInterface(b);
        return sService;
    }
