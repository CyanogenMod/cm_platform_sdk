...
public class CustomTile implements Parcelable {
    ...
    public CustomTile(Parcel parcel) {
        ...
        // the writeToParcel method. Then we step through each version, until the latest API release
    ...
    public static class ExpandedItem implements Parcelable {
        ...
        protected ExpandedItem(Parcel parcel) {
            ...
            // Pattern here is that all new members should be added to the beginning of
            // the writeToParcel method. Then we step through each version, until the
            // latest API release to help unravel this parcel
            if (parcelableVersion >= Build.CM_VERSION_CODES.APRICOT) {
                if (parcel.readInt() != 0) {
