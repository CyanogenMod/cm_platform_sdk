...
public class CMTelephonyManager {
    ...
    public boolean isSimActive(int subId) {
        ...
        try {
            simActive = sService.isSimActive(subId);
            if (localLOGD) {
                String pkg = mContext.getPackageName();
    ...
    public void setSimState(int subId, boolean active) {
        ...
        if (localLOGD) {
            String pkg = mContext.getPackageName();
            Log.v(TAG, pkg + " setting the state of the SIM with subscription " + subId + " as active: " + active);
        }

        try {
            sService.setSimState(subId, active);
        } catch (RemoteException e) {
            Slog.w(TAG, "warning: no cm telephony manager service");
        }
    }
    ...
    public boolean isDataConnectionSelectedOnSim(int subId) {
        ...
        try {
            dataConnectionActiveOnSim = sService.isDataConnectionSelectedOnSim(subId);
            if (localLOGD) {
                String pkg = mContext.getPackageName();
    ...
    public void setDataConnectionEnabled(boolean active) {
        ...
        if (localLOGD) {
            String pkg = mContext.getPackageName();
            Log.v(TAG, pkg + " setting the network data connection enabled: " + active);
        }

        try {
            sService.setDataConnectionEnabled(active);
        } catch (RemoteException e) {
            Slog.w(TAG, "warning: no cm telephony manager service");
        }
    }
    ...
    public void setDataConnectionSelectedOnSim(int subId) {
        ...
        try {
            sService.setDataConnectionSelectedOnSim(subId);
        } catch (RemoteException e) {
            Slog.w(TAG, "warning: no cm telephony manager service");
        }
    }
     ...
     *
     * If INVALID_SUBSCRIPTION_ID is used as a parameter, then the option to choose
     * what SIM to use is selected.
     *
     * @param subId The subscription to set as default for phone calls.
     *              To select SIM when calling use INVALID_SUBSCRIPTION_ID.
     */
    public void setPhoneCallUsingSim(int subId) {
        ...
        try {
            sService.setPhoneCallUsingSim(subId);
        } catch (RemoteException e) {
            Slog.w(TAG, "warning: no cm telephony manager service");
        }
    }
     ...
     *              To select SIM when sending SMS use INVALID_SUBSCRIPTION_ID.
     */
