...
public final class CMHardwareManager {
    ...
    public boolean writePersistentString(String key, String value) {
        try {
            return getService().writePersistentBytes(key, value.getBytes("UTF-8"));
        } catch (RemoteException e) {
     ...
     * @return true on success
     */
    public boolean writePersistentInt(String key, Integer value) {
        try {
     ...
     */
    public Integer readPersistentInt(String key) {
        try {
            byte[] bytes = getService().readPersistentBytes(key);
            if (bytes != null) {
