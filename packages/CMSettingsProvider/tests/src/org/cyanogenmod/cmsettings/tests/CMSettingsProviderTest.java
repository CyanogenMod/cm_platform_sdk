...
import java.util.Map;
 ...
 public class CMSettingsProviderTest extends AndroidTestCase {
     ...
     private static final String URI_FORMAT = "content://cmsettings/%s";

     private static final Uri SYSTEM_URI = Uri.parse(String.format(URI_FORMAT, "system"));
     private static final Uri SECURE_URI = Uri.parse(String.format(URI_FORMAT, "secure"));
     private static final Uri GLOBAL_URI = Uri.parse(String.format(URI_FORMAT, "global"));
     ...
     public void testBulkInsertSuccess() {
         ...
         testBulkInsertForUri(SYSTEM_URI, contentValues);
         testBulkInsertForUri(SECURE_URI, contentValues);
         testBulkInsertForUri(GLOBAL_URI, contentValues);
     ...
     private void testBulkInsertForUri(Uri uri, ContentValues[] contentValues) {
         ...
         // TODO Find a better way to cleanup database data ie use ProviderTestCase2 without process crash
     ...
     public void testInsertUpdateDeleteSuccess() {
         ...
         testInsertUpdateDeleteForUri(SYSTEM_URI);
         testInsertUpdateDeleteForUri(SECURE_URI);
         testInsertUpdateDeleteForUri(GLOBAL_URI);
     ...
     private void testInsertUpdateDeleteForUri(Uri uri) {
         ...
         int rowsAffected = mContentResolver.update(uri, contentValue, PROJECTIONS[0] + " = ?", new String[]{key1});
