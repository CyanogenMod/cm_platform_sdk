 ...
 * Audio sessions are created when a new AudioTrack is created, and
 * torn down when removed. Audio session identifiers are unique (but may
 * be reused) and are used as a handle to create audio effects on the
 * output stream.
 *
 * A list of global audio sessions can be obtained by calling
 * listAudioSessions() via CMAudioManager, or a component can listen
 * for the ACTION_AUDIO_SESSIONS_CHANGED broadcast.
 */
