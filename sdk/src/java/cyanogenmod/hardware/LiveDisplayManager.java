...
public class LiveDisplayManager {
    ...
    static final int MODE_FIRST = MODE_OFF;
    static final int MODE_LAST = MODE_DAY;
    ...
    static final int FEATURE_FIRST = FEATURE_CABC;
    static final int FEATURE_LAST = FEATURE_DISPLAY_MODES;
    ...
    public boolean isAutoContrastEnabled() {
        return mConfig.hasFeature(FEATURE_AUTO_CONTRAST) &&
               getInt(CMSettings.System.DISPLAY_AUTO_CONTRAST,
                      (mConfig.getDefaultAutoContrast() ? 1 : 0)) == 1;
    }
    ...
    public boolean setAutoContrastEnabled(boolean enabled) {
        if (!mConfig.hasFeature(FEATURE_AUTO_CONTRAST)) {
            return false;
        }
    ...
    public boolean isCABCEnabled() {
        return mConfig.hasFeature(FEATURE_CABC) &&
               getInt(CMSettings.System.DISPLAY_CABC,
                      (mConfig.getDefaultCABC() ? 1 : 0)) == 1;
    }
    ...
    public boolean isColorEnhancementEnabled() {
        return mConfig.hasFeature(FEATURE_COLOR_ENHANCEMENT) &&
               getInt(CMSettings.System.DISPLAY_COLOR_ENHANCE,
                      (mConfig.getDefaultColorEnhancement() ? 1 : 0)) == 1;
    }
    ...
    public int getDayTemperature() {
        return getInt(CMSettings.System.DISPLAY_TEMPERATURE_DAY,
                      mConfig.getDefaultDayTemperature());
    }
    ...
    public int getNightTemperature() {
        return getInt(CMSettings.System.DISPLAY_TEMPERATURE_NIGHT,
                      mConfig.getDefaultNightTemperature());
    }
    ...
    public boolean isAutomaticOutdoorModeEnabled() {
        return mConfig.hasFeature(MODE_OUTDOOR) &&
                (getInt(CMSettings.System.DISPLAY_AUTO_OUTDOOR_MODE,
                        (mConfig.getDefaultAutoOutdoorMode() ? 1 : 0)) == 1);
    }
    ...
    public boolean setColorAdjustment(float[] adj) {
        // sanity check
        if (!mConfig.hasFeature(FEATURE_COLOR_ADJUSTMENT) || adj.length != 3 ||
                adj[0] < 0 || adj[0] > 1.0f ||
                adj[1] < 0 || adj[1] > 1.0f ||
                adj[2] < 0 || adj[2] > 1.0f) {
            return false;
