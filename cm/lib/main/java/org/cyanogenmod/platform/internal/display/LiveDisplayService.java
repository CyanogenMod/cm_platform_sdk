...
public class LiveDisplayService extends SystemService {
    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_BOOT_COMPLETED) {
            ...
            final DisplayHardwareController dhc =
                    new DisplayHardwareController(mContext, mHandler);
            mFeatures.add(dhc);

            final ColorTemperatureController ctc =
                    new ColorTemperatureController(mContext, mHandler, dhc);
            mFeatures.add(ctc);

            final OutdoorModeController omc =
                    new OutdoorModeController(mContext, mHandler);
            mFeatures.add(omc);
            ...
            mConfig = new LiveDisplayConfig(capabilities, defaultMode,
                    ctc.getDefaultDayTemperature(), ctc.getDefaultNightTemperature(),
                    omc.getDefaultAutoOutdoorMode(), dhc.getDefaultAutoContrast(),
                    dhc.getDefaultCABC(), dhc.getDefaultColorEnhancement());
