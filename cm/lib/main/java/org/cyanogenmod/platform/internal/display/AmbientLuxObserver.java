...
public class AmbientLuxObserver {
            ...
            synchronized (AmbientLuxObserver.this) {
                switch (msg.what) {
                    case MSG_UPDATE_LUX:
                        ...
                        Log.d(TAG, "lux= " + lux + " mState=" + mState + " mAmbientLux=" + mAmbientLux);
                        mRingBuffer.add(lux);

                        direction = lux >= mThresholdLux ? HIGH : LOW;
                        if (mState != direction) {
                    ...
                    case MSG_TRANSITION:
                        ...
                        Log.d(TAG, "transition: mState= " + mState + " mAmbientLux=" + mAmbientLux);
                        ...
                        // check again when the window expires in case we didn't get any
                        // more readings because the sensor settled
                        if (mRingBuffer.size() > 1) {
                            sendEmptyMessageDelayed(MSG_TRANSITION, mThresholdDuration);
                        }
                        break;
                }
            }
    ...
    private static class TimedMovingAverageRingBuffer {
        ...
        public synchronized float getAverage() {
            ...
            return mRing.size() == 0 ? sum : sum / mRing.size();
        }

        public synchronized float[] getValues() {
            expire();
            float[] values = new float[mRing.size()];
            int j = 0;
            for (Iterator<Sample> i = mRing.iterator(); i.hasNext(); j++) {
                values[j] = i.next().mValue;
            }
            return values;
        }
        ...
        private void expire() {
            while (mRing.peek() != null &&
                    (System.currentTimeMillis() - mRing.peek().mTimestamp > mPeriod)) {
                mRing.pop();
            }
        }
        ...
        public synchronized String toString() {
            ...
            return "average=" + getAverage() + " mRing=[" + sb.toString() + "]";
        }
    }
}
