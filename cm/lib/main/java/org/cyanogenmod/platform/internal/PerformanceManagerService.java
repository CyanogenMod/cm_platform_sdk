...
public class PerformanceManagerService extends SystemService {
    ...
    private final int mPerfProfileDefault;
    ...
    public PerformanceManagerService(Context context) {
        ...
        mPerfProfileDefault = Integer.valueOf(context.getResources().getString(
                R.string.config_perf_profile_default_entry));
        mPerfProfileProperty = context.getResources().getString(
                R.string.config_perf_profile_prop);
        mProfilesEnabled = mPerfProfileDefault > -1 &&
            ...
            synchronized (this) {
                ...
                if (mProfilesEnabled) {
                    int profile = CMSettings.Secure.getInt(mContext.getContentResolver(),
                            CMSettings.Secure.PERFORMANCE_PROFILE, mPerfProfileDefault);
            ...
            PowerManagerInternal.LowPowerModeListener() {
                ...
                public void onLowPowerModeChanged(boolean enabled) {
                    ...
                    if (enabled && mCurrentProfile != PerformanceManager.PROFILE_POWER_SAVE) {
                        setPowerProfileInternal(Integer.valueOf(PerformanceManager.PROFILE_POWER_SAVE),
                                true);
                    } else if (!enabled && mPerfProfileDefault != mCurrentProfile) {
                        setPowerProfileInternal(mPerfProfileDefault, true);
                    }
                }
            };
}
