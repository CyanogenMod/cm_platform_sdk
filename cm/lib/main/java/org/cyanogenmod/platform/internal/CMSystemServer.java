...
public class CMSystemServer {
    ...
    private static final Map<Class, String> servicesToFeatures =
            new HashMap<>();

    /**
     * Correlates directly with config_externalCMServices and features declared per each
     * vertical stack. The feature declarations are within
     * {@link cyanogenmod.app.CMContextConstants.Features}.
     *
     ...
     */
    static {
        servicesToFeatures.put(CMStatusBarManagerService.class,
                CMContextConstants.Features.STATUSBAR);
        servicesToFeatures.put(ProfileManagerService.class,
                CMContextConstants.Features.PROFILES);
        servicesToFeatures.put(PartnerInterfaceService.class,
                CMContextConstants.Features.PARTNER);
        servicesToFeatures.put(CMTelephonyManagerService.class,
                CMContextConstants.Features.TELEPHONY);
        servicesToFeatures.put(CMHardwareService.class,
                CMContextConstants.Features.HARDWARE_ABSTRACTION);
        servicesToFeatures.put(AppSuggestManagerService.class,
                CMContextConstants.Features.APP_SUGGEST);
        servicesToFeatures.put(PerformanceManagerService.class,
                CMContextConstants.Features.PERFORMANCE);
        servicesToFeatures.put(ThemeManagerService.class,
                CMContextConstants.Features.THEMES);
        servicesToFeatures.put(IconCacheManagerService.class,
                CMContextConstants.Features.THEMES);
        servicesToFeatures.put(LiveLockScreenServiceBroker.class,
                CMContextConstants.Features.LIVE_LOCK_SCREEN);
        servicesToFeatures.put(CMWeatherManagerService.class,
                CMContextConstants.Features.WEATHER_SERVICES);
        servicesToFeatures.put(LiveDisplayService.class,
                CMContextConstants.Features.LIVEDISPLAY);
        servicesToFeatures.put(CMAudioService.class,
        ...
        for (String service : externalServices) {
            try {
                ...
                if (context.getPackageManager().hasSystemFeature(
                        servicesToFeatures.get(Class.forName(service)))) {
