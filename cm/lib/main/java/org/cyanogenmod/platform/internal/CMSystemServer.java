...
public class CMSystemServer {
    ...
    private static final Map<String, String> servicesToFeatures =
            new HashMap<>();

    /**
     * Correlates directly with config_externalCMServices and features declared per each
     * vertical stack.
     *
     * If you want to optionally disable an entire stack (service backend), simply delete the
     * feature.xml from system/etc/permissions.
     */
    static {
        servicesToFeatures.put(CMStatusBarManagerService.class.getCanonicalName(),
                CMContextConstants.Features.STATUSBAR);
        servicesToFeatures.put(ProfileManagerService.class.getCanonicalName(),
                CMContextConstants.Features.PROFILES);
        servicesToFeatures.put(PartnerInterfaceService.class.getCanonicalName(),
                CMContextConstants.Features.PARTNER);
        servicesToFeatures.put(CMTelephonyManagerService.class.getCanonicalName(),
                CMContextConstants.Features.TELEPHONY);
        servicesToFeatures.put(CMHardwareService.class.getCanonicalName(),
                CMContextConstants.Features.HARDWARE_ABSTRACTION);
        servicesToFeatures.put(AppSuggestManagerService.class.getCanonicalName(),
                CMContextConstants.Features.APP_SUGGEST);
        servicesToFeatures.put(PerformanceManagerService.class.getCanonicalName(),
                CMContextConstants.Features.PERFORMANCE);
        servicesToFeatures.put(ThemeManagerService.class.getCanonicalName(),
                CMContextConstants.Features.THEMES);
        servicesToFeatures.put(IconCacheManagerService.class.getCanonicalName(),
                CMContextConstants.Features.THEMES);
        servicesToFeatures.put(LiveLockScreenServiceBroker.class.getCanonicalName(),
                CMContextConstants.Features.LIVE_LOCK_SCREEN);
        servicesToFeatures.put(CMWeatherManagerService.class.getCanonicalName(),
                CMContextConstants.Features.WEATHER_SERVICES);
        servicesToFeatures.put(LiveDisplayService.class.getCanonicalName(),
                CMContextConstants.Features.LIVEDISPLAY);
        servicesToFeatures.put(CMAudioService.class.getCanonicalName(),
        ...
        for (String service : externalServices) {
            try {
                ...
                if (context.getPackageManager().hasSystemFeature(
                        servicesToFeatures.get(service))) {
