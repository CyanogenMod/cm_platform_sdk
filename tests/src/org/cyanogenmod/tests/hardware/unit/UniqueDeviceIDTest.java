...
public class UniqueDeviceIDTest extends AndroidTestCase {
    ...
    public void testGetSerialNumber() {
        ...
        if (mCMHardwareManager.isSupported(feature)) {
            String expectedSerialNo = SystemProperties.get("ro.serialno");
            String actualSerialNo = mCMHardwareManager.getSerialNumber();
            assertNotNull(actualSerialNo);
            assertEquals(expectedSerialNo, actualSerialNo);
        }
    }
    ...
    public void testGetUniqueDeviceId() {
        ...
        if (mCMHardwareManager.isSupported(feature)) {
            ...
            assertTrue(uniqueDeviceId.substring(0, 1).equals("0"));
            assertTrue(isHexNumber(uniqueDeviceId.substring(1, 3)));
        }
    }
    ...
    private static boolean isHexNumber(String target) {
        try {
